...
void BMotor::setup_mcpwm_configuration(int pwmFreq) {
    ...
    // mcpwm_sync_enable(MCPWM_UNIT_0, _hb0, MCPWM_SELECT_TIMER0_SYNC, 0);      // !!! MCPWM_SELECT_TIMER didn't exist in mcpwm.h, had to edit the header
    // mcpwm_sync_enable(MCPWM_UNIT_0, _hb1, MCPWM_SELECT_TIMER0_SYNC, 0);      // it sets to sync all timers to timer0
    // mcpwm_sync_enable(MCPWM_UNIT_0, _hb2, MCPWM_SELECT_TIMER0_SYNC, 0);      // see Github issue https://github.com/espressif/esp-idf/issues/5429
    ...
    mcpwm_sync_configure(MCPWM_UNIT_0, _hb0, &sync_conf);
    mcpwm_sync_configure(MCPWM_UNIT_0, _hb1, &sync_conf);
    mcpwm_sync_configure(MCPWM_UNIT_0, _hb2, &sync_conf);
